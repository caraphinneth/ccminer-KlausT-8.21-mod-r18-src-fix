...
extern void quark_bmw512_cpu_hash_80(int thr_id, uint32_t threads, uint32_t startNounce, uint32_t *d_hash);
...
//		CUDA_SAFE_CALL(cudaMalloc(&d_branch2Nonces, sizeof(uint32_t)*throughput));
	...
	quark_bmw512_cpu_setBlock_80(thr_id, (uint64_t *)endiandata);
	...
	do {
		...
		quark_bmw512_cpu_hash_80(thr_id, throughput, pdata[19], d_hash);
		quark_blake512_cpu_hash_64(thr_id, throughput, pdata[19], NULL, d_hash);
