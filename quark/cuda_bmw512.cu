...
__constant__ uint64_t c_PaddedMessage80[MAX_GPUS]; // padded message (80 bytes + padding)
...
__global__ // launch bounds slow this down? why...
...
__host__
void quark_bmw512_cpu_hash_80(int thr_id, uint32_t threads, uint32_t startNounce, uint32_t *d_hash)
{
...
void quark_bmw512_cpu_setBlock_80(int thr_id, void *pdata)
{
	...
	CUDA_SAFE_CALL(cudaMemcpyToSymbolAsync (c_PaddedMessage80, PaddedMessage, 16*sizeof(uint64_t), 0, cudaMemcpyHostToDevice));
}
